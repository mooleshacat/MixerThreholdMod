...
namespace ScheduleOne.Combat
{
	...
	{
		// Token: 0x06003362 RID: 13154 RVA: 0x000D5CEC File Offset: 0x000D3EEC
			...
			if (InstanceFinder.IsServer)
			{
				foreach (Collider collider in Physics.OverlapSphere(origin, num))
				{
					...
					if (componentInParent != null && !list.Contains(componentInParent))
					{
						...
						{
							if (!Physics.Raycast(origin, collider.transform.position - origin, ref hit, num, NetworkSingleton<CombatManager>.Instance.ExplosionLayerMask))
							{
